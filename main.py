...
class GameSprite(sprite.Sprite):
    ...
    def reset(self):
        window.blit(self.image, (self.rect.x, self.rect.y)) #Untuk menampilkan gambar ke window


class Player(GameSprite):
    def update_r(self): #Menggerakan raket di sebelah kanan
        keys = key.get_pressed()
        if keys[K_UP] and self.rect.y > 5:
            self.rect.y -= self.speed
        if keys[K_DOWN] and self.rect.y < win_height - 80:
            self.rect.y += self.speed
    def update_l(self): #Menggerakan raket di sebelah kiri
        keys = key.get_pressed()
...
window.fill(back) #Memberi warna background
